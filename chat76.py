...
#         if search_results:
#             return f"Sorry, I don't have a response for that. Here are some search results:\n{', '.join(search_results)}"
#         else:
...
def handle_input(user_input):
    ...
    if user_input in responses:
        ...
        search_results = list(search(user_input, num=3, stop=3, pause=2))
        if search_results:
            print("Opening browser and searching...")
            webbrowser.open(search_results[0], new=2)
            return f"Sorry, I don't have a response for that. Here are some search results:\n{', '.join(search_results)}"
        else:
            return "Sorry, I couldn't find any relevant information."
